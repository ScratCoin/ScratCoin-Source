...
contract ScratCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 16698200000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tDistributedRewardTotal;

    string private _name = "ScratCoin";
    ...
    uint256 private numTokenSwapThreshold = 1000000000 * 10**18;

    address deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public walletCharity = 0xD95860B2f912b529440A8e4D546cFAC8D301D332;
    address public walletMarketing = 0xa9496f92d940eb67b38F0eE6aE5F20de38FA5314;
    ...
    function _transferBothExcluded(
        ...
        (
            ...
            uint256 tReward,
            uint256 tBurnCharityAndMarketingFee
        ) = _getValues(tAmount);
    ...
    function setFeePercent(
        uint256 taxFee,
        uint256 burnFee,
        uint256 charityAndMarketingFee
    ) external onlyOwner {
        _rewardFee = taxFee;
        _burnFee = burnFee;
        ...
        )
    {
        ...
        uint256 tBurnCharityAndMarketingFee = calculateBurnCharityAndMarketingFee(
                tAmount
            );
        uint256 tTransferAmount = tAmount.sub(tReward).sub(
            tBurnCharityAndMarketingFee
        );
        ...
        )
    {
        ...
        uint256 rBurnCharityAndMarketingFee = tBurnCharityAndMarketingFee.mul(
            currentRate
        );
        uint256 rTransferAmount = rAmount.sub(rReward).sub(
            rBurnCharityAndMarketingFee
        );
    ...
    function _takeBurnCharityAndMarketingFee(
        uint256 tBurnCharityAndMarketingFee,
        address sender
    ) private {
        if (_burnFee + _charityAndMarketingFee > 0) {
            ...
            uint256 tBurn = tBurnCharityAndMarketingFee.mul(_burnFee).div(
                _burnFee + _charityAndMarketingFee
            );
            uint256 tCharityAndMarketingFee = tBurnCharityAndMarketingFee.sub(
                tBurn
            );
            ...
            uint256 rCharityAndMarketingFee = tCharityAndMarketingFee.mul(
                currentRate
            );
            _rOwned[address(this)] = _rOwned[address(this)].add(
                rCharityAndMarketingFee
            );
            if (_isExcluded[address(this)])
                _tOwned[address(this)] = _tOwned[address(this)].add(
                    tCharityAndMarketingFee
                );
    ...
    function calculateBurnCharityAndMarketingFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_burnFee + _charityAndMarketingFee).div(10**2);
    }

    function removeAllFee() private {
        if (_rewardFee == 0 && _burnFee == 0 && _charityAndMarketingFee == 0)
            return;
    ...
    function _transfer(
        ...
        if (launchTime == 0 && to == uniswapV2Pair) {
            ...
            if (
                ...
                block.timestamp < launchTime + 30 * 60
            ) {
                uint256 nTokenInPair = balanceOf(uniswapV2Pair);
                uint256 nBNBInPair = IERC20(uniswapV2Router.WETH()).balanceOf(
                    uniswapV2Pair
                );
                uint256 nTokenPerBNB = nTokenInPair.div(nBNBInPair).mul(10**18);
                require(
                    amount <= nTokenPerBNB / 2,
                    "In 30 min after launching, only number of token worth 0.5 BNB is allowed"
                );
            }
        }
    ...
    function swapAndSendOutFee(uint256 contractTokenBalance)
        private
        lockTheSwap
    {
